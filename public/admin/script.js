...
// ================== 【修改点】收件箱邮件卡片渲染 ==================
// 变更说明：在收件时间后面增加了收件人(receiver)的显示
// 原代码：${escapeHtml(m.sender)} | ${new Date(m.received_at).toLocaleString()}
// 新代码：${escapeHtml(m.sender)} | ${new Date(m.received_at).toLocaleString()} | 收件人: ${escapeHtml(m.receiver || '-')}
// 同时增加了 title 悬浮提示，方便查看完整地址
// 同时增加了 CSS 防止多个收件人时撑开布局
            ...
            html += `
                <div class="card mb-3 shadow-sm border-0">
                    <div class="card-body p-3">
                        ...
                        <h6 class="card-subtitle mb-2 text-muted small" style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;" title="发件人: ${escapeHtml(m.sender)}&#10;时间: ${new Date(m.received_at).toLocaleString()}&#10;收件人: ${escapeHtml(m.receiver || '-')}">${escapeHtml(m.sender)} | ${new Date(m.received_at).toLocaleString()} | 收件人: ${escapeHtml(m.receiver || '-')}</h6>
